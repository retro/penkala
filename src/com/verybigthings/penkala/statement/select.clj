...
(defn add-projection [acc db relation]
  (let [projection (reduce-kv
                     (fn [projection _ join-relation]
                       (into projection (get-projection db join-relation)))
                     (get-projection db relation)
                     (:relation/joins relation))]
    (update acc :sql conj (str/join ", " projection))))
  ...
  (reduce-kv
    (fn [acc' _ j]
      (let [join-kind (:join/kind j)
            join-kind-sql ({:inner "INNER JOIN"} join-kind)
            join-on (:join/on j)
